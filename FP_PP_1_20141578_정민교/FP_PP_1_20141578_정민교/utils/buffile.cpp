...
int BufferFile::Read (unsigned char& bufSize, unsigned char& skipSize, int recaddr)
...
{
	if (recaddr == -1)
		return Buffer . Read (File, bufSize, skipSize);
	else
		return Buffer . DRead (File, recaddr);
}
 
int BufferFile::Write (int recaddr, int skip)
 // write the current buffer contents
{
	if (recaddr == -1)
		return Buffer . Write (File, skip);
	else
